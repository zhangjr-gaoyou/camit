...
2) 如果是，按在整份试卷中的阅读顺序抽取内容，并对每一项标注类型：
   ...
   - "答题说明"：紧跟在某个板块标题之后、与板块标题在同一行括号中的**整板块**说明文字，例如「(本题共 12 小题，每小题 2 分……)」。**注意**：若为「请用楷书抄写下面的文字……（3分）」这类**单道题的作答要求**，且紧接一段待抄写/待填空的正文（如「天边的红霞，向晚的微风……」），则整道题为**一条** type=题目、subtype=填空题，content 中先写该作答要求，换行后写待抄写/待填空的正文，不要拆成答题说明+题干；
...
7) 请根据常见题型，对 type=题目 的 content 做结构化组织（仅在 content 内体现题型含义，不改变 type 字段）：
   ...
   - 填空题：若为「抄写题」（如「请用楷书抄写下面的文字……（3分）」+ 待抄写段落「天边的红霞……」），只输出**一条** type=题目、subtype=填空题，content 为作答要求 + 换行 + 待抄写/待填空的正文。若为「一题干+多填空小题项」（如「在（）里填上">"、"<"或"="。」下列若干比较式），content 格式为：第一行写题干，随后每行一条填空小题，用换行分隔。其他填空题：题干中所有需要填写的空位用下划线或【填空】_____【/填空】标出，不要给出正确答案。
...
- 填空题「抄写题」示例：「请用楷书抄写下面的文字……（3分）」+ 段落「天边的红霞，向晚的微风……绘成了一幅自然、和谐的田园风景画。」只输出**一条**题目，type=题目、subtype=填空题，content 为：「请用楷书抄写下面的文字，注意书写正确、工整，布局合理。（3分）\\n\\n天边的红霞，向晚的微风……」（不要拆成答题说明+题干）。
...
【重要】请特别注意：1) 题干与题目必须严格区分——问句单独标为题干，含 A/B/C/D 选项的整体标为题目；2) **「请用楷书抄写下面的文字……（3分）」+ 待抄写段落 = 一条 题目/填空题，不要拆成答题说明+题干；** 每道小题单独一条 items；填空题若为一题干+多个填空小题项，只输出一条题目；3) type=题目 时必须标注 subtype（选择题/填空题/简答题/计算题/匹配题等）；4) 数学公式、化学方程式必须用 LaTeX 格式 $...$ 包裹；5) 田字格、米字格等填空格一律用下划线或【填空】_____【/填空】表示；6) 每项的 bbox 须完整覆盖该内容在图片中的区域；7) 当题目选项为图形（[图片A] 等）时，bbox 必须覆盖题干及所有选项图片的完整区域。
"""
...
/// 学习分析报告提示词：基于错题信息生成分析报告
func learningAnalysisPrompt(wrongQuestionsSummary: String) -> String {
    """
    你是一位经验丰富的学习辅导老师。请根据以下学生的错题记录，生成一份简洁、结构清晰的学习分析报告。

    错题记录已按科目分组：
    \(wrongQuestionsSummary)

    **Markdown 格式规范（必须严格遵守）：**
    1. 科目标题：使用 ## 科目名（如 ## 语文、## 数学），不要用 **加粗** 代替。
    2. 章节标题：使用 ### 一、知识薄弱点分析、### 二、学习建议、### 三、针对性练习题，不要用 **加粗** 代替。
    3. 每个 ## 科目标题后空一行，再写内容；每个 ### 章节标题后空一行，再写内容。
    4. 正文中的重点词可用 **加粗**，但标题一律用 ## 或 ###。

    每个科目标题下，按以下结构输出（保持简洁，每项 1～2 句话）：

    ### 一、知识薄弱点分析
    2～4 个薄弱点，每点一行：**知识点名称** + 简要表现（1 句话）。
    ...
    **【知识点】xxx**
    ...
    数学公式用 LaTeX：$E=mc^2$、$\\frac{1}{2}$。报告直接输出，不要用代码块包裹。
    """
}
