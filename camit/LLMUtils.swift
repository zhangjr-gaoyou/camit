...
【重要】请特别注意：1) 题干与题目必须严格区分——问句单独标为题干，含 A/B/C/D 选项的整体标为题目；2) 每道小题必须单独作为一条 items，不可合并（如「1.」与「2.」为两道题）；3) type=题目 时必须标注 subtype（选择题/填空题/简答题/计算题/匹配题等）；4) **板块分类仅当试卷显式写出板块标题（如一、选择题、二、填空题）时才输出，不要根据题型推断添加**；5) 数学公式、化学方程式必须用 LaTeX 格式 $...$ 包裹；6) 田字格、米字格等填空格一律用下划线或【填空】_____【/填空】表示；7) 每项的 bbox 须完整覆盖该内容在图片中的区域；8) 当题目选项为图形（[图片A] 等）时，bbox 必须覆盖题干及所有选项图片的完整区域；9) **bbox 与 option_bboxes 必须按矩形输出**：{\"x\":起点x,\"y\":起点y,\"width\":宽度,\"height\":高度}，x/y 为左上角起点，width/height 可不等（矩形），首次解析即输出，后续据此切图并精修。
"""
