...
/// 从题目内容开头解析原题序号（如 "7. (3分)把不等式组..." -> 7，"11. 填空..." -> 11）
func extractQuestionNumber(from content: String) -> Int? {
...
2) 如果是，按在整份试卷中的阅读顺序抽取内容，并对每一项标注类型：
   - "板块分类"：**仅当试卷中显式出现**大题板块标题时才输出，如「一、选择题」「二、填空题（每小题3分，共24分）」等。若试卷中未显式写出板块名（如只有题号 7、8、9… 而无「一、选择题」等标题），则不要输出板块分类，题型通过题目的 subtype 体现即可；
...
6) **图形处理**：a) 当选择题的选项为图形（如图标、示意图）而非文字时，content 中用 [图片A]、[图片B]、[图片C]、[图片D] 占位符，bbox 覆盖题干+所有选项。若能识别各选项的独立区域，请增加 option_bboxes 字段。b) 当题干或题目中出现「如图」且页面上有对应示意图时，请紧邻其后输出一条 type=\"附图\" 的项，content 为空，bbox 为附图边界框。**bbox 与 option_bboxes 必须按矩形输出**：{\"x\":起点x,\"y\":起点y,\"width\":宽度,\"height\":高度}。x、y 为左上角起点坐标（归一化 0-1），width、height 为矩形的宽和高（可不等，即矩形不必为正方形）。首次解析即输出该格式，后续将据此切图并精修。
...
type 可为 "板块分类"、"答题说明"、"题干"、"题目"、"附图" 之一。附图仅用于「如图」引用的示意图。当 type=题目 时，必须包含 subtype 字段。题目若含图形选项可增加 option_bboxes。
bbox 格式：{\"x\":起点x,\"y\":起点y,\"width\":宽度,\"height\":高度}，全部为归一化坐标（0-1）。x、y 为左上角起点，width、height 为矩形宽高（可为不同值，不必为正方形）。当题目选项为图形时，bbox 覆盖题干+所有选项；若有 option_bboxes 则可分别切出各选项。首次解析即输出该矩形坐标，后续将据此切图并精修。
"""
...
【重要】请特别注意：
1) 题干与题目必须严格区分——问句单独标为题干，含 A/B/C/D 选项的整体标为题目；
2) 每道小题必须单独作为一条 items，不可合并（如「1.」与「2.」为两道题）；
3) type=题目 时必须标注 subtype（选择题/填空题/简答题/计算题/匹配题等）；
4) **当 subtype=选择题 时，无论是否提供 option_bboxes，你都必须在 content 中完整逐行写出所有可见文字选项，格式为「A. …」「B. …」「C. …」「D. …」。绝对不能只写题干而省略选项；如果上一次回答中漏掉了选项，这一次必须补全。**
5) **板块分类仅当试卷显式写出板块标题（如「一、选择题」「二、填空题」）时才输出，不要根据题型推断添加**；
6) 数学公式、化学方程式必须用 LaTeX 格式 $...$ 包裹；
7) 田字格、米字格等填空格一律用下划线或【填空】_____【/填空】表示；
8) 每项的 bbox 须完整覆盖该内容在图片中的区域；
9) 当题目选项为图形（[图片A] 等）时，bbox 必须覆盖题干及所有选项图片的完整区域；
10) **bbox 与 option_bboxes 必须按矩形输出**：{\"x\":起点x,\"y\":起点y,\"width\":宽度,\"height\":高度}，x/y 为左上角起点，width/height 可不等（矩形），首次解析即输出，后续据此切图并精修。
"""
